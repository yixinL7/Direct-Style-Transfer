...
class ParaModel(nn.Module):
    ...
    def scoring_approximate(self, g_idxs1, g_mask1, g_lengths1, g_idxs2):

        g1 = self.encode(g_idxs1, g_mask1, g_lengths1)
        g2 = self.approximate(g_idxs2)
        return self.cosine(g1, g2)

class WordAveraging(ParaModel):

    def __init__(self, args, vocab, weights=None):
        ...
        if weights is None:
            ...
            self.embedding = nn.Embedding(len(self.vocab), self.args.dim, _weight=weights)
        if args.gpu >= 0:
           self.cuda()
